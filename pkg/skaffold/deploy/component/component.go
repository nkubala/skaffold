...
func (c ComponentProvider) GetKubernetesLogger(podSelector *kubernetes.ImageList, cli *kubectl.CLI) log.Logger {
	return logger.NewLogAggregator(cli, podSelector, c.config)
}
...
func (c ComponentProvider) GetKubernetesSyncer(cli *kubectl.CLI) sync.Syncer {
	return sync.NewPodSyncer(cli, c.config)
}
